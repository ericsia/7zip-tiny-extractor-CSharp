...
namespace pdj.tiny7z.Archive
{
    internal class SevenZipStreamsExtractor : Compression.IPasswordProvider
    {
        ...
        internal SevenZipStreamsExtractor(Stream stream, SevenZipHeader.StreamsInfo streamsInfo)
        {
            this.stream = stream;
        ...
        internal void Extract(ulong outStreamIndex, Stream outStream)
        {
            ExtractMultiple(new ulong[] { outStreamIndex }, (ulong i) => outStream, null);
        }

        internal void ExtractMultiple(ulong[] outStreamIndices, Func<ulong, Stream> onStreamRequest, Action<ulong, Stream> onStreamClose)
        {
            // simplify
        ...
        private void extractMultipleFromFolder(ulong outputStreamIndexOffset, bool[] matches, ulong folderIndex, ulong packIndex, Func<ulong, Stream> onStreamRequest, Action<ulong, Stream> onStreamClose)
        {
            using (var decoder = createDecoderStream(folderIndex, packIndex))
            {
                ...
                Util.TransferTo(decoder, outStream, (long)neededUnPackSize);
